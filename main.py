...
# Load dictionary (must be named bangla_dictionary.json)
...
def get_bengali_from_dict(word):
    """Try multiple strategies to find the word in the dictionary."""
    if not word:
        return None
    ...
    # Direct match (exact, lower, capitalised, upper)
    ...
    # Try substring match (if word appears inside a key)
...
async def run_daily_job():
    ...
    # ---- DICTIONARY LOOKUP ----
    if bengali_dict:
        print("🔍 Looking up meanings in dictionary (smart matching)...")
        dict_hits = 0
        for item in vocab:
            word = item.get("word", "")
            if not word:
